package com.example.wpy.teamattention.download;
...
import static com.example.wpy.teamattention.download.SharedPreferencedUtils.getLong;
        ...
        try {
            ...
            mDownloadCall.enqueue(new Callback() {
                ...
                public void onResponse(Call call, Response response) {
                    ...
                    try {
                        is = body.byteStream();
                        long contentLength = getLong(mContext, "contentLength", 0L);
                        //这里的Length为每次请求下来的大小,并不是文件本身的总大小,所以为了计算下载进度,这里只取第一次文件的总大小
                        if (contentLength <= 0L) {
